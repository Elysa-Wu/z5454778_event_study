...
def get_tics(pth):
    ...
    with open(pth, mode='r') as fobj:
        for All_tickers in fobj:
            All_tickers = All_tickers.strip().lower().replace('"', '')
            separate_tickers = All_tickers.split('=')
            if len(separate_tickers) == 2:
                ExchangeMarket  = separate_tickers[0].strip()
                shortticker = separate_tickers[1].strip()
                if shortticker.isalpha() and ExchangeMarket.isalpha() and ' ' not in ExchangeMarket and ' ' not in shortticker:
                    tickers_dic [shortticker] = ExchangeMarket
...
def line_to_dict(line):
    ...
    for Key in COLUMNS:
        widths = COLWIDTHS[Key]
        Value = line[start:start + widths].strip()
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    for ticker in tickers_lst:
        if ticker.lower() not in tic_exchange_dic:
            raise Exception(f"tickers_lst contains a ticker ({ticker}) that doesn't correspond to a key tic_exchange_dic")
...
def verify_cols(col_lst=None):
    ...
    for column in col_lst:
        if column not in COLUMNS:
            raise Exception(f"col_lst contains a column ({column} that isn't found in `COLUMNS`")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for ticker in tickers_lst:
        ...
        data_rows = read_dat(ticker)

        for row in data_rows:
            line_dict = {col: line_to_dict(row).get(col) for col in col_lst if col in ['Date', 'Close']}
...
Explain which one of the 2 hypotheses is more likely to be true:


Evaluate the short-run predictability for the trading volume with reason:
